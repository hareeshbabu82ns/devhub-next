...
const DevotionalQuickAccess: React.FC<DevotionalQuickAccessProps> = ({
  ...
  const handleTileClick = (entity: Entity) => {
    // For now, navigate directly to entity since parents is just an array of IDs
  ...
  const todayCategory = DAY_INDEX_TO_CATEGORY[todayIndex];
  const todayAssociation = isDaySpecificCategory(todayCategory)
    ? DAY_DEITY_ASSOCIATIONS[todayCategory]
    : {
        deity: "All Deities",
        color: "text-gray-500",
        ...
        <CardContent className="pb-4">
          <TabsContent value="everyday">
            <div className="space-y-4">
              ...
              {everydayError && (
                <SimpleAlert title="Error loading everyday content" />
              )}
              ...
              {everydayData && everydayData.results.length === 0 && (
                <div className="text-center py-8 text-muted-foreground">
                  ...
                  <p className="text-sm">
                    Use the search button above to add STHOTRAM and PURANAM
                    entities to your daily collection.
                  </p>
                </div>
              )}

              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
                {everydayData?.results.map((entity) =>
                  renderEntityTile(entity),
                )}
              </div>
            </div>
          </TabsContent>
          ...
          <TabsContent value="daily">
            <div className="space-y-4">
              ...
              {dayLoading && <Loader />}
              {dayError && (
                <SimpleAlert title="Error loading day-specific content" />
              )}
              ...
              {dayData && dayData.results.length === 0 && (
                <div className="text-center py-8 text-muted-foreground">
                  ...
                  <p>
                    ...
                    {
                      DAYS_OF_WEEK[
                        Object.values(DAY_INDEX_TO_CATEGORY).indexOf(
                          selectedDay,
                        )
                      ]
                    }
                  ...
                  <p className="text-sm">
                    Use the search button above to add STHOTRAM and PURANAM
                    entities to this day's collection.
                  </p>
                </div>
              )}
