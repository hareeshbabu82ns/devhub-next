...
describe("DictionaryRepository", () => {
  ...
  describe("findWords", () => {
    ...
    it("should filter by queryText with KEY_PREFIX mode", async () => {
      (mockPrismaClient.dictionaryWord.findMany as jest.Mock).mockResolvedValue(
        [mockWords[0]],
      );
      (mockPrismaClient.dictionaryWord.count as jest.Mock).mockResolvedValue(1);
    ...
    it("should filter by queryText with KEY_EXACT mode", async () => {
      (mockPrismaClient.dictionaryWord.findMany as jest.Mock).mockResolvedValue(
        [mockWords[0]],
      );
      (mockPrismaClient.dictionaryWord.count as jest.Mock).mockResolvedValue(1);
    ...
    it("should delegate to aggregateSearch for FULLTEXT mode", async () => {
      ...
      // Should call aggregateRaw instead of findMany
      expect(mockPrismaClient.dictionaryWord.aggregateRaw).toHaveBeenCalled();
      expect(mockPrismaClient.dictionaryWord.findMany).not.toHaveBeenCalled();
    });
    ...
    it("should handle pagination with hasMore", async () => {
      ...
      (mockPrismaClient.dictionaryWord.count as jest.Mock).mockResolvedValue(
        10,
      );
  ...
  describe("aggregateSearch", () => {
    it("should perform full-text search with aggregation", async () => {
      ...
      expect(result.hasMore).toBe(false);
      expect(
        mockPrismaClient.dictionaryWord.aggregateRaw,
  ...
  describe("findById", () => {
    it("should find word by ID", async () => {
      ...
      (
        mockPrismaClient.dictionaryWord.findUnique as jest.Mock
  ...
  describe("error handling", () => {
    it("should propagate database errors", async () => {
      ...
      (mockPrismaClient.dictionaryWord.findMany as jest.Mock).mockRejectedValue(
        dbError,
      );
      ...
      await expect(repository.findWords(query)).rejects.toThrow(
